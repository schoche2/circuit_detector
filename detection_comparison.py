...
#TFLITE addition
TFLITE_MODEL_PATH = "circuit_model_v2.tflite"
...
img_raw = cv2.imread('dataset/circuits/20210311_144631.jpg')
...
def predict_objects(img_raw):
    ...
    for i in range(len(components)):
        ...
        # Grayscale preprocessing with channel check
        ...
        tf_image = resized.astype("float32")
        #img_square = np.expand_dims(img_square, axis=-1)  # add channel dimension
        #img_square = np.expand_dims(img_square, axis=0)   # add batch dimension
        tf_image = tf_image.reshape(1, 128, 128, 1)  # Make sure input shape matches model

        # Keras prediction
        #prediction = model.predict(tf_image)

        # TFLite prediction
        interpreter.set_tensor(input_index, tf_image)
        ...
        #tf_image = cv2.normalize(img_square, None, 255, 0, cv2.NORM_MINMAX, cv2.CV_8U)

        #interpreter.set_tensor(input_index,tf_image)
        interpreter.invoke()
        tflite_output = interpreter.get_tensor(output_index)
        tfllite_label = CLASS_NAMES[np.argmax(tflite_output)]

        component_objects_tflite.update({i + 1 :
                                    {
                                        'type': tfllite_label,
                                        'location' : square,
        ...
        print("KERAS" , component_objects)
        print("TFLITE" , component_objects_tflite)
